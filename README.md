...
- src/iterative_pruning.py: 核心剪枝与多维蒸馏实现。
    
- src/quantization.py: W8A16 量化器与反量化线性层定义。
    
- src/speculative_decoder.py: 基于 Top-K 采样的大-小模型投机解码器。
    
- src/pld_model.py: 基于 N-Gram 匹配的预测查找解码实现（适配 OpenCompass）。
...
### 3. 自适应投机推理
```bash
python adaptive_framework/run_inference.py --input "请帮我写一段关于量子计算的科普文字"
```
